...
export default function PintuMasuk() {
  ...
  // FUNGSI LOGIN PENGECEKAN NYATA + AKUN ADMIN BERSAMA
  const handleLogin = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      // 1. CEK LOGIKA AKUN ADMIN BERSAMA (Gunakan 'admin' untuk ID dan Password)
      if (loginId.trim().toLowerCase() === 'admin' && loginPass === 'admin') {
      ...
      // 2. JIKA BUKAN ADMIN, CEK DATABASE LOKAL USER BIASA
      const usersDB = JSON.parse(localStorage.getItem('db_users') || '[]');
